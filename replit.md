...
- Required env: `DATABASE_URL`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHANNEL_ID`; DEXTools discovery additionally requires `DEXTOOLS_API_KEY`.
...
- DEXTools uses its documented V2 `hotpools`, `gainers`, and token-detail endpoints with the official `X-API-Key` header. It polls a conservative default chain set and can be narrowed with `DEXTOOLS_CHAINS`.
- Pump.fun has an explicitly experimental no-key watcher that polls public Solana JSON-RPC endpoints with bounded fallback, identifies `create` and `create_v2` instructions using the official Pump.fun program definition, and enriches only from the on-chain metadata URI. It is not treated as an official Pump.fun discovery API.
...
- Unsupported or unconfigured discovery feeds are represented as disabled provider modules with an explicit reason rather than simulated events or guessed endpoints. Experimental feeds are labeled as such and do not inherit production reliability claims.
...
The console shows provider health, live discovery totals, recent token events, token metadata, chain coverage, duplicate suppression, and Telegram delivery state. Settings support pause/resume and optional filters. Pump.fun is visible as an experimental public-RPC watcher; direct Solana discovery remains disabled. DEXTools remains visibly disabled until `DEXTOOLS_API_KEY` is set.
